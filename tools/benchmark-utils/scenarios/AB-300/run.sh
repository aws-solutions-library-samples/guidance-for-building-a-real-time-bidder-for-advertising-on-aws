...
image_version="AB-300"

enable_profiler=0
#rate_cases=(20000 40000 60000)
rate_cases=(60000)
duration="5m"
...
function make_bidder() {
    if (($# < 2)); then
        ...
        echo "  usage: make_bidder stack_name variant_name"
        exit 1
    fi
    ...
    kubectl patch deployment bidder -p "$(cat "$dir/deployment.patch.yaml")"
    reload_bidders
}
...
function run_benchmark() {
    if (($# < 2)); then
        ...
        echo "  usage: run_benchmark variant_name scenario_name"
        exit 1
    fi
    ...
    for rate in "${rate_cases[@]}"; do
        _log "running scenario ( variant=${variant_name} rate=${rate} )"
        ...
        make_bidder "$stack_name" "${variant_name}"
...
run_benchmark "Benchmark" "${benchmark_datetime}-default-cpu-policy"
...
run_benchmark "BenchmarkStaticCPU" "${benchmark_datetime}-static-cpu-policy"
